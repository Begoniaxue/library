...
export async function seedData() {
  ...
  const weekEnd = new Date(monday.setDate(monday.getDate() + 6)).toISOString().split('T')[0]
  ...
  try {
    ...
    for (const s of students) {
      const result = await studentService.add(s)
      createdStudents.push(result)
    }
    
    if (createdStudents.length === 0) {
      console.warn('Failed to seed data: no students created')
      return
    }
    
    for (const k of knowledges) {
      await knowledgeService.add(k)
    }
